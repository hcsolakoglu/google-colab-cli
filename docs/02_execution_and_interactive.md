---
log:
2026-09-24: Redacted interactive stdin replies from persistent session history. Runtime execution still receives the original reply, but history records only `<redacted>` so auth codes/password-like input are not written to disk.
