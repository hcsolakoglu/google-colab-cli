---
log:
2026-09-24: Hardened local credential handling: OAuth token/debug-log files are created with private permissions, sensitive request/response headers and token fields are redacted, and urllib3 wire DEBUG logging is disabled because Contents API URLs carry the runtime proxy token as a query parameter.
